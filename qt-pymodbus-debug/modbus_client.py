...
class ModbusFrameHandler(logging.Handler):
    def __init__(self, client):
        super().__init__()
        self.client = client  # referensi ke ModbusClient
    ...
    def emit(self, record):
        ...
        if "SEND:" in msg or "RECV:" in msg:
            try:
                # Ambil teks setelah "SEND:" atau "RECV:"
                parts = msg.split(":", 1)
                if len(parts) < 2:
                    return
                data_part = parts[1]

                # Ambil hanya token yang valid: 0x## atau hex digit
                hex_tokens = []
                ...
                # Debug, bisa kamu aktifkan kalau mau lihat parsing error
...
class ModbusClient:
    ...
    def read_holding(self, address, count, unit=1):
        return self.client.read_holding_registers(address, count, unit=unit)

    def read_input(self, address, count, unit=1):
        return self.client.read_input_registers(address, count, unit=unit)

    def read_coils(self, address, count, unit=1):
        return self.client.read_coils(address, count, unit=unit)

    def read_discrete(self, address, count, unit=1):
        return self.client.read_discrete_inputs(address, count, unit=unit)

    def write_register(self, address, value, unit=1):
        return self.client.write_register(address, value, unit=unit)

    def write_registers(self, address, values, unit=1):
        return self.client.write_registers(address, values, unit=unit)
