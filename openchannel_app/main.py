...
from PyQt5.QtCore import Qt, QPointF, QTimer, QThread, QObject, pyqtSignal
...
class SchematicWidget(QWidget):
    ...
    def _draw_trapezoidal(self, p):
        ...
        if is_cipolletti:
            slope_text = "Kemiringan sisi (tetap - Cipolletti)\nhorizontal : vertikal = 1 : 4"
        else:
            slope_text = f"Kemiringan sisi\nhorizontal : vertikal = 1 : {1 / z:.2f}  (z = {z:g})"
...
# =============================================================================
# MODBUS WORKER (dijalankan di thread terpisah dari GUI)
# =============================================================================

class ModbusWorker(QObject):
    """Menjalankan satu siklus baca+decode Modbus di thread terpisah.

    Pembacaan Modbus RTU (serial) bersifat blocking - bisa menunggu sampai
    nilai timeout tercapai kalau perangkat tidak merespons. Kalau ini
    dijalankan langsung di thread GUI (mis. dari dalam QTimer.timeout di
    MainWindow), seluruh jendela aplikasi akan freeze/lag selama menunggu.
    Worker ini dipindah ke QThread terpisah (lihat MainWindow.__init__)
    sehingga GUI tetap responsif; hasilnya dikirim balik ke GUI thread lewat
    sinyal Qt (otomatis di-queue lintas-thread oleh Qt, jadi aman)."""

    read_finished = pyqtSignal(dict)

    def __init__(self, reader):
        super().__init__()
        self.reader = reader

    def read_once(self, params):
        """params: dict berisi slave_id, address, count, function_code,
        data_type, swap_words, swap_bytes, scale, offset, unit, kind, tag."""
        try:
            registers = self.reader.read_raw_registers(
                params["slave_id"], params["address"], params["count"], params["function_code"]
            )
            if registers is None:
                self.read_finished.emit({
                    "ok": False, "kind": params["kind"], "tag": params.get("tag"),
                    "error": self.reader.last_error or "Gagal membaca register.",
                })
                return
            raw_value = decode_registers(
                registers, params["data_type"],
                swap_words=params["swap_words"], swap_bytes=params["swap_bytes"],
            )
            converted = raw_value * params["scale"] + params["offset"]
            self.read_finished.emit({
                "ok": True, "kind": params["kind"], "tag": params.get("tag"),
                "registers": registers, "raw_value": raw_value,
                "converted": converted, "unit": params["unit"],
            })
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # --- export data (langsung ke file, tanpa riwayat sesi di memori) ---
        ...
        self.last_export_sample_ts = None

        # Modbus I/O (koneksi & baca register) dijalankan di thread terpisah
        # supaya UI tidak freeze/lag saat polling - operasi serial bersifat
        # blocking (bisa menunggu sampai timeout jika perangkat tidak
        # merespons), dan menjalankannya di thread GUI akan membekukan
        # seluruh jendela selama itu.
        self.modbus_thread = QThread(self)
        self.modbus_worker = ModbusWorker(self.modbus_reader)
        self.modbus_worker.moveToThread(self.modbus_thread)
        self.modbus_thread.start()
        self.request_read.connect(self.modbus_worker.read_once)
        self.modbus_worker.read_finished.connect(self._on_read_finished)
        ...
        self.calc_btn.clicked.connect(lambda: self.calculate())
        self.reset_btn.clicked.connect(self.reset_form)
        self.choose_export_btn.clicked.connect(self.choose_export_file)
        self.disable_export_btn.clicked.connect(self.disable_export_file)
    ...
    def _build_menu(self):
        ...
        export_action = QAction("Pilih/Ganti File Export...", self)
        export_action.triggered.connect(self.choose_export_file)
        file_menu.addAction(export_action)

        clear_action = QAction("Nonaktifkan Export Otomatis", self)
        clear_action.triggered.connect(self.disable_export_file)
    ...
    def _build_result_panel(self):
        ...
        # ---- Export & riwayat ----
        export_caption = QLabel("EXPORT DATA PERHITUNGAN")
        export_caption.setObjectName("resultCaption")
        layout.addWidget(export_caption)

        self.export_file_label = QLabel("File export: (belum dipilih - hasil tidak disimpan ke file)")
        self.export_file_label.setObjectName("historyLabel")
        self.export_file_label.setWordWrap(True)
        layout.addWidget(self.export_file_label)

        self.export_count_label = QLabel("")
        self.export_count_label.setObjectName("resultSub")
        layout.addWidget(self.export_count_label)

        export_hint = QLabel(
            "Setiap kali \u201cHitung\u201d ditekan (atau H terisi dari sensor Modbus), "
            "hasilnya langsung ditambahkan (append) ke file di bawah ini - dibuat "
            "otomatis kalau belum ada, tidak perlu tombol export terpisah."
        )
        export_hint.setWordWrap(True)
        export_hint.setObjectName("descLabel")
        layout.addWidget(export_hint)

        export_row = QHBoxLayout()
        self.choose_export_btn = QPushButton("Pilih/Ganti File Export...")
        self.choose_export_btn.setObjectName("exportBtn")
        self.disable_export_btn = QPushButton("Nonaktifkan")
        self.disable_export_btn.setObjectName("clearHistBtn")
        export_row.addWidget(self.choose_export_btn, stretch=3)
        export_row.addWidget(self.disable_export_btn, stretch=2)
    ...
    def _build_mapping_group(self):
        ...
        form.addRow("Urutan Word/Byte (32/64-bit):", self.mb_wordorder_combo)
    ...
    def _build_control_group(self):
        ...
        self.mb_poll_export_checkbox = QCheckBox("Simpan Sampel Polling ke File Export")
        layout.addWidget(self.mb_poll_export_checkbox)

        export_interval_form = QFormLayout()
        export_interval_form.setSpacing(8)
        self.mb_poll_export_interval_input = QDoubleSpinBox()
        self.mb_poll_export_interval_input.setRange(1.0, 3600.0)
        self.mb_poll_export_interval_input.setDecimals(1)
        self.mb_poll_export_interval_input.setValue(10.0)
        self.mb_poll_export_interval_input.setSuffix(" s")
        self.mb_poll_export_interval_input.setEnabled(False)
        export_interval_form.addRow("Interval Simpan ke File:", self.mb_poll_export_interval_input)
        layout.addLayout(export_interval_form)

        self.mb_poll_export_checkbox.toggled.connect(self.mb_poll_export_interval_input.setEnabled)

        export_note = QLabel(
            "Bila File Export sudah dipilih (lihat tab Kalkulasi), opsi ini "
            "menambahkan baris hasil polling (H, Q, mass flow, dst.) langsung "
            "ke file tersebut. Karena polling bisa berjalan sangat cepat, "
            "hanya satu sampel yang ditambahkan per interval di atas (bukan "
            "setiap siklus polling)."
        )
        export_note.setWordWrap(True)
        export_note.setObjectName("descLabel")
    ...
    def toggle_connection(self):
        if not self.modbus_reader.connected:
            ...
            # Membuka koneksi serial adalah aksi sekali-klik (bukan siklus
            # berulang seperti polling), jadi dilakukan langsung di GUI
            # thread - penundaan singkat saat membuka port bisa diterima
            # dan lebih sederhana daripada menambah round-trip ke worker
            # thread untuk operasi yang cuma terjadi sesekali.
            ...
            if self.poll_in_flight:
                QMessageBox.information(
                    self, "Mohon Tunggu",
                    "Sedang menunggu hasil pembacaan Modbus yang berjalan. Coba lagi sesaat lagi."
                )
                return
            if self.is_polling:
                self.toggle_polling()
    ...
    def _current_mapping(self, kind="manual"):
    ...
    def perform_single_read(self):
        if not self.modbus_reader.connected:
            ...
            return
        if self.poll_in_flight:
            self.status_label.setText("Sedang menunggu pembacaan sebelumnya selesai, coba lagi sesaat.")
            return
        self.poll_in_flight = True
        self.mb_read_once_btn.setEnabled(False)
        self.status_label.setText("Membaca sensor Modbus...")
        # Pembacaan sesungguhnya dijalankan di worker thread (lihat
        # ModbusWorker) supaya UI tidak freeze menunggu respons perangkat;
        # hasilnya ditangani di _on_read_finished() saat sinyal diterima.
        self.request_read.emit(self._current_mapping(kind="manual"))
    ...
    def poll_once(self):
        """Dipanggil oleh QTimer setiap interval polling. Hanya MEMINTA
        pembacaan ke worker thread (non-blocking) - hasilnya ditangani
        secara async di _on_read_finished()."""
        if self.poll_in_flight:
            # Siklus sebelumnya belum selesai (perangkat lambat merespons /
            # interval polling terlalu singkat) - lewati tick ini daripada
            # menumpuk permintaan baca.
            return
        self.poll_in_flight = True
        self.request_read.emit(self._current_mapping(kind="poll"))

    def _on_read_finished(self, result):
        """Slot yang menerima hasil dari ModbusWorker (jalan di GUI thread,
        Qt otomatis meng-antre sinyal lintas-thread jadi ini aman)."""
        self.poll_in_flight = False
        kind = result.get("kind")

        if kind == "manual":
            self.mb_read_once_btn.setEnabled(True)

        if not result.get("ok"):
            err = result.get("error") or "Error tidak diketahui."
            if kind == "manual":
                QMessageBox.warning(self, "Gagal Membaca Sensor", err)
                self.status_label.setText("Gagal membaca sensor.")
            else:
                self.consecutive_poll_errors += 1
                self.status_label.setText(f"Polling gagal ({self.consecutive_poll_errors}x): {err}")
        ...
        if kind == "manual":
            ...
            # yang sedang aktif di tab Kalkulasi (calculate() juga langsung
            # menambahkannya ke file export bila sudah diatur).
        ...
        status_extra = ""
        # Simpan sebagian hasil polling langsung ke File Export, dibatasi
        # dengan interval sampling (bukan setiap siklus) supaya file tidak
        # dibanjiri saat interval polling sangat cepat (bisa serendah 200ms).
        if Q_val is not None and self.mb_poll_export_checkbox.isChecked():
            if not self.export_file_path:
                status_extra = " (File Export belum dipilih di tab Kalkulasi - sampel tidak disimpan)"
            else:
                sample_interval_s = self.mb_poll_export_interval_input.value()
                now = datetime.now()
                elapsed = (
                    (now - self.last_export_sample_ts).total_seconds()
                    if self.last_export_sample_ts is not None else None
                )
                if elapsed is None or elapsed >= sample_interval_s:
                    params = self._get_param_values()
                    entry = self._build_row_dict(
                    ...
                    if self._append_row_to_export_file(entry):
                        status_extra = " - sampel ditambahkan ke file export."
                    self.last_export_sample_ts = now

        self.status_label.setText(f"Polling: H = {converted:,.3f} {unit} - {now_str}{status_extra}")
    ...
    def _rebuild_param_form(self, params):
        ...
        for spec in params:
            ...
            self.param_form.addRow(label_widget, widget)
    ...
    def calculate(self, sumber="Manual (Kalkulasi)"):
        try:
            ...
            entry = self._build_row_dict(type_name, method_name, params, H_val, H_unit, H_m, rho,
                                          Q, area, notes, warnings, sumber=sumber)
            saved = self._append_row_to_export_file(entry)
            suffix = " - ditambahkan ke file export." if saved else ""
            if not saved and not self.export_file_path:
                suffix = " (File Export belum dipilih - hasil tidak disimpan ke file)."
            self.status_label.setText(
                f"Perhitungan berhasil - {type_name} / {method_name} - "
                f"{datetime.now().strftime('%H:%M:%S')}{suffix}"
            )
        except Exception as e:
    ...
    def _build_row_dict(self, type_name, method_name, params, H_val, H_unit, H_m, rho,
    ...
    def _update_export_label(self):
        if not self.export_file_path:
            self.export_file_label.setText("File export: (belum dipilih - hasil tidak disimpan ke file)")
            self.export_count_label.setText("")
        else:
            self.export_file_label.setText(f"File export: {self.export_file_path}")
            self.export_count_label.setText(
                f"{self.exported_row_count} baris ditambahkan ke file ini pada sesi ini."
            )

    # ------------------------------------------------------------------
    # Export - setiap hasil perhitungan langsung ditambahkan (append) ke
    # file yang dipilih, bukan dikumpulkan dulu ke riwayat sesi lalu
    # diexport manual belakangan. File dibuat otomatis kalau belum ada.
    # ------------------------------------------------------------------
    def choose_export_file(self):
        default_name = f"hasil_openchannel_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        fname, _ = QFileDialog.getSaveFileName(
            self, "Pilih atau Buat File Export", default_name,
        ...
        if not (fname.lower().endswith(".xlsx") or fname.lower().endswith(".csv")):
            fname += ".xlsx"
        if fname.lower().endswith(".xlsx") and not OPENPYXL_AVAILABLE:
            QMessageBox.warning(
                self, "Modul Tidak Tersedia",
                "Export ke Excel (.xlsx) memerlukan modul 'openpyxl' yang belum "
                "terpasang.\n\nInstall dengan:\n    pip install openpyxl\n\n"
                "Atau pilih nama file berakhiran .csv sebagai alternatif."
            )
            return
        self.export_file_path = fname
        self.exported_row_count = 0
        self._update_export_label()
        note = "akan ditambahkan (append) ke file yang sudah ada" if os.path.exists(fname) else \
               "file baru akan dibuat saat hasil pertama ditambahkan"
        self.status_label.setText(f"File export diatur ke: {fname} ({note}).")

    def disable_export_file(self):
        self.export_file_path = None
        self.exported_row_count = 0
        self._update_export_label()
        self.status_label.setText("Export otomatis dinonaktifkan. Hasil perhitungan tidak lagi disimpan ke file.")

    def _append_row_to_export_file(self, entry):
        """Menambah satu baris ke file export aktif: append kalau file sudah
        ada, atau membuat file baru (lengkap dengan header) kalau belum ada.
        Mengembalikan True kalau berhasil, False kalau tidak ada file export
        yang diatur atau terjadi kesalahan (pesan error masuk ke status bar)."""
        if not self.export_file_path:
            return False
        try:
            if self.export_file_path.lower().endswith(".csv"):
                self._append_row_csv(self.export_file_path, entry)
            else:
                self._append_row_xlsx(self.export_file_path, entry)
            self.exported_row_count += 1
            self._update_export_label()
            return True
        except Exception as e:
            self.status_label.setText(f"Gagal menambah baris ke file export: {e}")
            return False

    def _append_row_csv(self, fname, entry):
    ...
    def _append_row_xlsx(self, fname, entry):
        pretty_headers = self._XLSX_PRETTY_HEADERS
        ...
        for col_idx, key in enumerate(self.HISTORY_FIELDS, start=1):
            ws.cell(row=next_row, column=col_idx, value=entry.get(key, ""))
        wb.save(fname)
    ...
    def closeEvent(self, event):
        """Membersihkan koneksi Modbus, timer polling, dan thread worker saat aplikasi ditutup."""
        try:
            if self.is_polling:
                self.poll_timer.stop()
