...
def calc_trap_cipolletti(H, p, rho):
    """Cipolletti weir standar: kemiringan sisi tetap 1 (H) : 4 (V).
    Q = 1.859 * L * H^1.5  (satuan metrik: L,H dalam meter, Q dalam m3/s)
    """
    L = p["L"]
    z = 0.25  # kemiringan sisi standar Cipolletti (horizontal per vertikal)
...
def _p(key, label, unit="-", default=0.0, minv=-1e9, maxv=1e9, decimals=3, step=0.01, ptype="double", options=None, default_index=0):
    ...
    "Trapezoidal Weir (Weir Trapesium)": {
        "methods": {
            "Cipolletti (Standar, Sisi 1:4)": {
                ...
                "desc": "Q = 1.859\u00b7L\u00b7H^1.5 (kemiringan sisi tetap 1 horizontal : 4 vertikal)"
            },
            "Trapesium Umum (Kemiringan Kustom)": {
                ...
                "params": [
                    ...
                    _p("Cd_rect", "Cd Bagian Persegi", unit="-", default=0.620, minv=0.400, maxv=0.900, decimals=3, step=0.001),
                    _p("Cd_tri", "Cd Bagian Segitiga", unit="-", default=0.600, minv=0.400, maxv=0.900, decimals=3, step=0.001),
                ],
                "desc": "Kombinasi bagian persegi + 2 bagian segitiga dengan kemiringan sisi z"
            },
        }
    },
    "Parshall Flume": {
